"""
Generate nomic-results.jsonl by running all benchmark queries through the
Nomic embedding search pipeline.
...
_HERE      = Path(__file__).resolve().parent          # nomic-embedding/evaluation/
_REPO_ROOT = _HERE.parent.parent                      # project root
sys.path.insert(0, str(_REPO_ROOT))

_spec = importlib.util.spec_from_file_location(
    "nomic_query", _HERE.parent / "query.py"
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate nomic-results.jsonl from benchmark queries.")
    parser.add_argument("--queries", default=str(_REPO_ROOT / "evaluation" / "benchmark_queries.jsonl"))
    parser.add_argument("--output",  default=str(_REPO_ROOT / "evaluation" / "nomic-results.jsonl"))
